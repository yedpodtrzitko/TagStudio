...
class ModelAction(enum.StrEnum):
    FOLDER = "FOLDER"
    ACTION = "ACTION"
...
class DropImportModal(QWidget):
    ...
    def ask_folder(self):
        ...
        # if len(folders) == 1: self.target_folder = folders[0]
        if len(folders) > 0:  # TODO > 1
            # create a popup window with all folders in the library
            ...
            # trigger event on double click
            self.list_widget.itemDoubleClicked.connect(self.on_folder_selected)

    def on_folder_selected(self, item):
        # get user role data
        folder_id = item.data(Qt.ItemDataRole.UserRole)
        # get the folder object
        folders = self.driver.lib.get_folders()
        for folder in folders:
        ...
        self.import_urls(self.urls)

    def import_urls(self, urls: list[QUrl]):
        """Add a collection of urls to the library."""
        ...
        for url in urls:
            ...
            if file.is_dir():
                for f in file.glob("**/*"):
                    ...
                    self.files.append(f)
                    if (self.target_folder.path / self._get_relative_path(file)).exists():
                ...
                if (self.target_folder.path / file.name).exists():
    ...
    def ask_duplicates_choice(self):
        ...
        self.desc_widget.setText(
            f"The following {len(self.duplicate_files)} file(s) have filenames already exist in the library."
            # noqa: E501
        )
    ...
    def copy_files(self):
        ...
        for file in self.files:
            ...
            (self.target_folder.path / dest_file).parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(file, self.target_folder.path / dest_file)
