...
class Library:
    ...
    def __init__(self) -> None:
        ...
        self.missing_matches: dict[Path, typing.Any] = {}
        # Duplicate Files
    ...
    def create_library(self, path: Path | str) -> int:
        """
    ...
    def verify_ts_folders(self) -> None:
        ...
        if not full_ts_path.is_dir():
            os.mkdir(full_ts_path)

        if not full_backup_path.is_dir():
            os.mkdir(full_backup_path)

        if not full_collage_path.is_dir():
    ...
    def refresh_dupe_files(self, results_filepath) -> None:
        """
